...
DEFAULT_SCOPES = ["graph.read", "people.read", "graph.admin"]
...
async def get_current_user(
    ...
    Test/dev path: decode payload without verification (matches Blocks F/G stub).
    """
    if credentials is None or not credentials.credentials:
        if settings.environment == "test":
            return {
                "tenant_id": "tenant_h_test",
                "principal_id": "user_h_test",
...
def require_scopes(*required: str):
    ...
    async def _check(
        ...
        # Accept scope strings or space-delimited oauth scope claim
