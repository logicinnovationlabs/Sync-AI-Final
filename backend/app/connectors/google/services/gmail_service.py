...
class GmailConnector(BaseConnector):
    ...
    async def transform(self, raw_documents: List[Dict[str, Any]]) -> List[UnifiedDocument]:
        ...
        for message in raw_documents:
            ...
            # Permissions: mailbox owner. Prefer live mailbox email over placeholder.
            mailbox = self.mailbox_email or message.get("_mailbox_email") or ""
            permissions = [f"user:{mailbox}"] if mailbox else ["user:*"]
