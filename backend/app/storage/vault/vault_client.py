...
class TableVaultBackend(VaultBackend):
    ...
    def get(self, key_ref: str) -> Optional[str]:
        ...
        Returns:
            Secret value as string (unwrapped if plain value), or None if not found
        """
        query = "SELECT value_jsonb FROM secrets WHERE key_ref = %s"
        result = self.db_client.fetch_one(query, (key_ref,))
        
        if result:
            value_jsonb = result["value_jsonb"]
            # If it's a wrapped plain string ({"value": "..."}), unwrap it
            if isinstance(value_jsonb, dict) and len(value_jsonb) == 1 and "value" in value_jsonb:
                return value_jsonb["value"]
            # Otherwise return as JSON string
            return json.dumps(value_jsonb)
        return None
    ...
    def set(self, key_ref: str, value: str):
        ...
        Args:
            key_ref: The key reference identifier
            value: The secret value (can be JSON string or plain string)
        """
        # Try to parse as JSON, if it fails, wrap it in a JSON object
        try:
            value_json = json.loads(value)
        except json.JSONDecodeError:
            # Plain string value, wrap it in a JSON envelope
    ...
    def rotate(self, key_ref: str, new_value: str):
        ...
        query = """
            ...
            WHERE key_ref = %s
        """
        self.db_client.execute(query, (value_json, key_ref))
        logger.info(f"Rotated secret for key_ref: {key_ref}")
